...
- il browser non tenta più di aprire/ricaricare la pagina quando viene rilasciata una card;
...
## Drag fix hardening v3

Ulteriore correzione per evitare ricariche indesiderate durante il drag & drop:

- prevenzione del comportamento nativo in capture phase su `document` e `window`;
- protezione aggiuntiva sul contenitore principale dell'app;
- dropzone con `onDragOverCapture`, `onDropCapture` e payload custom `application/prioro-task-id`;
- pulsanti interni alle card esclusi dal drag.
