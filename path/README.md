...
- Settimana estesa da lunedì a domenica.
- Layout principale in blocco unico: `Nuova attività`, `Elenco attività`, `Giorni della settimana`.
- Da mobile: elenco attività sopra alla settimana e navigazione giorni tramite swipe reale, con un giorno alla volta visibile.
- Doppio click su attività in elenco o nei giorni per modificarne il testo.
- Pulsante `Svuota` su ogni giorno per rimettere tutte le attività di quel giorno nell'elenco.
- Obiettivi, categorie e archivio spostati sotto il blocco operativo principale.

Se il database era già creato, riesegui `supabase/schema.sql` per aggiornare il check dei giorni con `SABATO` e `DOMENICA`.
...
- L'elenco attività da trascinare è sopra ai giorni e non usa più scorrimento orizzontale da mobile.

## Correzione mobile elenco attività

Da mobile l'elenco delle attività da trascinare non usa più schede grandi verticali una sotto l'altra. Ora usa una griglia compatta:

- 3 colonne sui telefoni standard;
- 2 colonne su schermi molto stretti;
- 4 colonne su schermi mobile più larghi.

Le card sono più compatte per rendere l'elenco meno dispersivo prima della selezione dei giorni.

## Fix stabile drag & drop

Questa versione riparte dalla build mobile compatta funzionante e sostituisce il drag & drop con una gestione più stabile:

- niente listener invasivi in capture phase;
- uso di `useRef` per mantenere l'id dell'attività trascinata anche se React aggiorna lo stato;
- payload sicuro `prioro-task:<id>` invece del solo UUID;
- `preventDefault` e `stopPropagation` solo nelle vere dropzone;
- il drop legge prima il payload, poi il riferimento interno;
- i pulsanti interni alle card non avviano il drag.
