...
Questa versione mantiene la logica principale di PrioWeek e rimuove temporaneamente gli obiettivi.
...
- Rimossa la doppia scritta `PrioWeek` nell'header: resta solo il titolo principale.
...
Il pulsante sotto il calendario ora scarica direttamente un PDF (`prioweek-planner-settimanale.pdf`) invece di aprire la stampa del browser.
...
- Il titolo `PrioWeek` e il pulsante hamburger restano sempre fissi in alto.
...
- Design minimal coerente con PrioWeek.
...
- Se non ci sono ancora spostamenti tracciati, PrioWeek mostra ancora la stima sulle attività aperte da oltre 7 giorni.
...
## Correzione v15 - Premium PrioWeek Score minimal

- Aggiunta metrica interna `PrioWeek Score` nella sezione Statistiche Premium.
...
- Aggiunta FAQ finale per spiegare significato di KPI, grafici, PrioWeek Score, streak, trend, heatmap e procrastinazione.
- Nessun nuovo SQL richiesto.
...
- PrioWeek Score, KPI Premium, trend, heatmap produttività e heatmap procrastinazione sono stati forzati a larghezza piena.
...
## Versione v34 - Rebrand PrioWeek

- Sostituito il nome prodotto con `PrioWeek`.
- Aggiornato titolo pagina, header, landing, testi Premium, PDF, Excel e report email.
- Aggiornato `Weeko Score` in `PrioWeek Score`.
- Aggiornati nomi file export: `prioweek-attivita.xls` e `prioweek-planner-settimanale.pdf`.
- Aggiornato package name in `prioweek`.
- Nessun SQL richiesto.

## Versione v35 - Layout statistiche più leggibile

- Modificata solo l'impaginazione dei grafici a doppia barra nella pagina Statistiche.
- Ora ogni riga mostra prima il nome del giorno/categoria e sotto le barre.
- Struttura: `[NOME GIORNO/CATEGORIA]` + barra completate + barra rimandate/spostate.
- Mantenuti invariati calcoli, KPI, colori, Premium, export, planner e tutte le funzionalità precedenti.
- Nessun SQL richiesto.

## Versione v36 - Wizard primo utilizzo

- Aggiunto mini tutorial guidato al primo accesso dell'utente.
- Il wizard si apre automaticamente solo la prima volta per ogni utente/browser.
- Il completamento viene salvato in `localStorage` con chiave dedicata all'utente.
- Aggiunto pulsante `Tutorial` nell'header desktop e nel menu mobile per rivederlo manualmente.
- Il wizard guida l'utente tra Planner, creazione attività, drag & drop, Statistiche e Premium.
- Mantenute tutte le funzionalità esistenti, incluso layout statistiche v35, check attività e rebrand PrioWeek.
- Nessun SQL richiesto.

## Versione v37 - Pulizia note e home più ricca

- Rimossa la nota tecnica sui pagamenti nella pagina Premium.
- Rimosso il pulsante Tutorial da desktop e menu mobile.
- Mantenuto il wizard automatico al primo accesso, senza pulsante manuale visibile.
- Rimossa la nota sugli insight/PrioWeek Score dalla pagina Statistiche.
- Aggiunta una Home dashboard più completa sopra la creazione attività:
  - saluto utente;
  - PrioWeek Score;
...
- Mantenute tutte le funzionalità precedenti.
- Nessun SQL richiesto.

## Versione v38 - Planner subito visibile

- Rimosso il blocco Home dashboard dalla pagina Planner.
- La pagina Planner ora parte direttamente da "Crea nuova attività" e prosegue con elenco attività e calendario.
- Mantenute tutte le funzionalità precedenti: wizard automatico primo accesso, statistiche, premium, ricorrenze, check attività, export, rollover e rebrand PrioWeek.
- Nessun SQL richiesto.

## Versione v39 - Home page pubblica ampliata

- Ampliata la landing/home page pubblica prima del login.
- Aggiunte sezioni descrittive:
  - Perché PrioWeek;
  - flusso elenco attività, settimana, futuro e statistiche;
  - come funziona in 3 passaggi;
  - confronto Free/Premium;
  - CTA finale più forte.
- Mantenuto invariato il planner interno, che parte direttamente da "Crea nuova attività".
- Nessun SQL richiesto.

## Versione v42 - Landing demo-first

- Landing pubblica semplificata e più professionale.
- Rimossa la landing lunga e descrittiva.
- Demo interattiva fake sopra la piega:
  - elenco attività trascinabile;
  - giorni LUN, MAR, MER, GIO, VEN;
  - drop delle attività nei giorni;
  - possibilità di riportare le attività nell'elenco;
  - reset demo.
- KPI demo laterali:
  - PrioWeek Score;
  - Attività completate;
  - Procrastinazione.
- Login/registrazione spostati sotto la demo in forma più compatta.
- CTA finale essenziale.
- Planner interno invariato: dopo login parte ancora direttamente da "Crea nuova attività".
- Nessun SQL richiesto.

## Versione v43 - Demo generica, responsive e home arricchita

- Rimossi riferimenti specifici come VSL dalla demo.
- Demo aggiornata con attività generiche e comprensibili per tutti.
- Demo resa più stabile e responsive:
  - KPI spostati sotto la demo;
  - colonne con larghezza minima controllata;
  - scroll orizzontale su tablet quando serve;
  - impaginazione verticale su mobile;
  - testi delle card gestiti senza tagli o overflow.
- Aggiunta sezione "Cosa puoi fare".
- Aggiunta sezione "Per chi è pensato".
- Mantenuto invariato il planner interno.
- Nessun SQL richiesto.

## Versione v44 - Home pulita e accesso in modale

- Rimossa la sezione "Cosa puoi fare" perché ridondante rispetto alla demo.
- Rimossa la sezione Accesso inline dalla home.
- Login e registrazione ora si aprono in una modale pulita dai pulsanti Accedi, Inizia gratis e CTA finale.
- Aggiunto blocco "Prima di iniziare" più coerente e professionale.
- Mantenute demo interattiva, KPI, sezione destinatari e CTA finale.
- Planner interno invariato.
- Nessun SQL richiesto.

## Versione v45 - Orario opzionale attività

- Aggiunto campo orario opzionale sulle attività (`scheduled_time`).
- Nel form di creazione attività è disponibile un input orario.
- In modifica attività è possibile assegnare, cambiare o rimuovere l'orario.
- Le attività pianificate dentro i giorni vengono ordinate automaticamente per orario.
- Le attività senza orario restano dopo quelle con orario.
- L'orario viene mostrato come badge dentro la card pianificata.
- Export Excel aggiornato con colonna Orario.
- PDF del planner aggiornato con orario accanto al titolo attività.
- Aggiunta migrazione SQL: `supabase/add_task_scheduled_time_v45.sql`.

### SQL da eseguire se il database Supabase esiste già
...
Se invece ricrei il database da zero, lo schema completo `supabase/schema.sql` è già aggiornato.

## Versione v46 - Check da elenco attività conteggiato nel giorno corrente

- Quando completi una attività ancora presente nell'elenco attività, l'app la assegna automaticamente al giorno corrente prima di marcarla come completata.
- Esempio: se oggi è venerdì e completi una attività dal backlog, viene salvata come `Fatto` e `VENERDI`.
- Le statistiche giornaliere leggono quindi correttamente il completamento nel giorno giusto.
- Se invece l'attività era già pianificata in un giorno del calendario, mantiene quel giorno.
- Nessuna nuova migrazione SQL richiesta rispetto alla V45.

## Versione v47 - Procrastinazione solo per attività già pianificate

- La procrastinazione ora viene contata solo quando un'attività aveva già un giorno di calendario assegnato e viene poi spostata/rimandata.
- Le attività non pianificate, cioè quelle ancora nell'elenco attività, non aumentano più la procrastinazione quando vengono completate o inserite nel calendario.
- Il check da elenco attività continua a conteggiare la task nel giorno corrente come introdotto in V46.
- Le statistiche Premium sono state allineate: tasso procrastinazione, heatmap, barre per giorno e categorie usano solo i rimandi reali.
- Nessuna nuova migrazione SQL richiesta rispetto alla V45.
