...
function PremiumStatsBlock({ stats, isPremium, reportSettings, toggleWeeklyReport }) {
  ...
  return <div className="panel premium-panel review-wide premium-stats-block">
    ...
    {!isPremium ? <p className="stat-muted">Disponibili con Premium: analisi avanzate, trend, report settimanale via email e storico completo.</p> : <>
      <p className="stat-muted">Blocco Premium provvisorio: lo teniamo pulito sotto ai grafici e poi decidiamo insieme quali insight rendere davvero utili.</p>
      <div className="advanced-stats-grid"><div><strong>{bestCategory}</strong><span>Categoria dominante</span></div><div><strong>{stats.topDayLabel}</strong><span>Giorno migliore</span></div><div><strong>{reportSettings?.weekly_email_enabled ? 'Attivo' : 'Non attivo'}</strong><span>Report email</span></div></div>
