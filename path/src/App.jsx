...
function Prioro({ session }) {
  ...
  function downloadPlannerPdf() {
    ...
    days.forEach((day, i) => {
      ...
      drawCardList(day, items, x + 2, dayTop + 5, colW - 4, colH - 6, 15, 'line');
    });

    const bottomTop = dayTop + colH + 9;
    const blockGap = 4;
    const blockW = (contentW - blockGap) / 2;
    roundedRect(margin, bottomTop, blockW, 49, 2, '#ffffff', '#ffffff');
    drawCardList('OGNI GIORNO', every, margin, bottomTop + 3, blockW, 44, 6, 'card');
    roundedRect(margin + blockW + blockGap, bottomTop, blockW, 49, 2, '#ffffff', '#ffffff');
    drawCardList('FUTURO', future, margin + blockW + blockGap, bottomTop + 3, blockW, 44, 6, 'card');
  ...
  return <main className="app-shell">
    ...
    </> : <StatsPage stats={stats} statsPeriod={statsPeriod} setStatsPeriod={setStatsPeriod} categoryMap={categoryMap} />}
...
function StatsPage({ stats, statsPeriod, setStatsPeriod, categoryMap }) {
