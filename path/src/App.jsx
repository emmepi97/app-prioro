...
const premiumPdfTemplateName = 'PDF Premium compatto';
...
function Prioro({ session }) {
  ...
  function downloadPlannerPdf() {
    ...
    text(isPremium ? 'Weeko Premium - Planner settimanale' : 'Weeko - Planner settimanale', margin, 12, { size: 14, bold: true });
...
function PremiumPlannerTools({ isPremium, categories, newRecurring, setNewRecurring, addRecurringTask, recurringTasks, deleteRecurringTask, generateRecurringTasks }) {
  return <section className="premium-tools-panel">
    <div className="premium-tools-head"><div><p className="eyebrow">Premium</p><h2>Automazioni</h2><p>Le ricorrenze sono salvate qui, visibili, eliminabili e generabili con controllo anti-duplicati.</p></div><span className={isPremium ? 'premium-badge active' : 'premium-badge'}>{isPremium ? 'Attivo' : 'Bloccato'}</span></div>
    {isPremium ? <div className="premium-tools-grid single-template-grid">
      ...
      <div className="template-box"><strong>PDF Premium automatico</strong><p>Nel piano Premium il pulsante Scarica PDF usa automaticamente il template premium compatto. Non crea attività e non crea categorie.</p><span className="premium-badge active">{premiumPdfTemplateName}</span></div>
      <div className="recurring-list-box">
        <div className="recurring-list-head"><strong>Ricorrenze attive</strong><button type="button" onClick={() => generateRecurringTasks(recurringTasks)}>Genera settimana</button></div>
        {recurringTasks.length ? <div className="recurring-list">{recurringTasks.map(item => <div className="recurring-row" key={item.id}><div><b>{item.title}</b><span>{item.recurrence_day}</span></div><button type="button" onClick={() => deleteRecurringTask(item.id)}>Elimina</button></div>)}</div> : <div className="archive-empty">Nessuna ricorrenza creata.</div>}
      </div>
    </div> : <div className="premium-locked-note">Sblocca Premium per ricorrenze, PDF premium automatico, report email, statistiche evolute e attività condivise.</div>}
  </section>;
}
...
function PremiumStatsBlock({ stats, isPremium, reportSettings, toggleWeeklyReport }) {
  ...
  return <div className="panel premium-panel review-wide premium-stats-block">
    ...
    {!isPremium ? <p className="stat-muted">Disponibili con Premium: trend, report settimanale via email, storico completo e insight ricorrenti.</p> : <>
      <div className="advanced-stats-grid"><div><strong>{bestCategory}</strong><span>Categoria dominante</span></div><div><strong>{stats.topDayLabel}</strong><span>Giorno migliore</span></div><div><strong>{stats.completed}</strong><span>Completate periodo</span></div><div><strong>{stats.open}</strong><span>Aperte periodo</span></div><div><strong>{weeklyAvg}</strong><span>Media stimata settimanale</span></div><div><strong>{stats.completionRate}%</strong><span>Completamento periodo</span></div></div>
