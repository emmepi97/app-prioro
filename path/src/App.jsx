...
function CategoryStats({ rows }) { const max = Math.max(1, ...rows.map(r => r.value)); return <div className="category-stats monochrome-category-stats">{rows.length ? rows.map(r => <div className="category-stat" key={r.label}><span><i />{r.label}</span><b>{r.value}</b><div><i style={{ width: `${Math.max(4, (r.value / max) * 100)}%` }} /></div></div>) : <div className="archive-empty">Nessuna categoria completata.</div>}</div>; }
