...
function getOverdueDayKeys(date = new Date()) {
  ...
  if (todayIndex === 0) return ['DOMENICA'];
  ...
  if (!session) {
    return <div className="landing-page">
      <header className="landing-nav">
        <div className="landing-logo">Prioro</div>
      ...
      <section className="landing-hero">
        <div className="landing-copy">
          <p className="eyebrow">Planner settimanale minimal</p>
          <h1>La settimana è piena. Il problema è capire cosa conta davvero.</h1>
          <p className="landing-lead">Prioro ti aiuta a trasformare attività sparse, promemoria e cose da fare in una settimana chiara, visibile e gestibile in una sola pagina.</p>
          <div className="landing-cta-row">
            <button type="button" className="primary" onClick={() => setMode('register')}>Crea il tuo planner</button>
            <button type="button" className="soft-btn" onClick={() => setMode('login')}>Ho già un account</button>
          </div>
          <div className="landing-microcopy">Free per iniziare. Premium per ricorrenze, statistiche evolute e attività condivise.</div>
        </div>

        <form className="auth-card landing-auth-card" onSubmit={submit}>
          <p className="eyebrow">{mode === 'login' ? 'Accesso' : 'Registrazione'}</p>
          <h2>{mode === 'login' ? 'Entra in Prioro' : 'Crea account'}</h2>
          <p>{mode === 'login' ? 'Accedi al tuo planner settimanale.' : 'Crea il tuo planner e inizia a organizzare la settimana.'}</p>
      ...
      <section className="landing-section problem-section">
        <div className="landing-section-head">
          ...
          <h2>Le attività aumentano, ma la visione resta frammentata.</h2>
        </div>
        <div className="landing-card-grid problem-grid">
          <div><strong>Non sai dove iniziare</strong><span>Hai attività sparse tra testa, note, chat e calendario.</span></div>
          <div><strong>La settimana non è visibile</strong><span>Vedi tante cose da fare, ma non capisci subito il carico dei giorni.</span></div>
          <div><strong>Le cose ricorrenti si dimenticano</strong><span>Ogni settimana riscrivi attività simili o le recuperi all’ultimo.</span></div>
        </div>
      </section>
      ...
      <section className="landing-section solution-section">
        <div className="landing-section-head">
          ...
          <h2>Una sola pagina per pianificare, spostare e rivedere.</h2>
        </div>
        <div className="landing-solution-layout">
          <div className="solution-list">
            <div><b>Planner settimanale</b><span>Trascini le attività nei giorni e hai subito la settimana sotto controllo.</span></div>
            <div><b>Futuro e Ogni giorno</b><span>Separi cosa fare ora, cosa ricordare dopo e cosa ripetere spesso.</span></div>
            <div><b>Statistiche e archivio</b><span>Capisci cosa completi, quali categorie pesano di più e come evolve il tuo lavoro.</span></div>
            <div><b>Premium</b><span>Ricorrenze, report email, statistiche avanzate e attività condivise.</span></div>
          </div>
          <div className="landing-product-preview">
            <div className="preview-top"><span>Planner</span><span>Statistiche</span><span>Premium</span></div>
            <div className="preview-week">{days.slice(0,5).map(d => <div key={d}><strong>{dayLabels[d]}</strong><i /></div>)}</div>
            <div className="preview-bottom"><span>OGNI GIORNO</span><span>FUTURO</span></div>
          </div>
        </div>
      </section>
      ...
      <section className="landing-final-cta">
        ...
        <h2>Apri Prioro, svuota la testa e costruisci la tua settimana.</h2>
        <p>Parti gratis. Se il planner entra nella tua routine, puoi sbloccare Premium quando ti serve.</p>
...
function Prioro({ session }) {
  ...
  const [taskEditDraft, setTaskEditDraft] = useState({ title: '', category_id: '', priority: '', notes: '', shared_email: '' });
  ...
  function cancelEditTask() {
    ...
    setTaskEditDraft({ title: '', category_id: '', priority: '', notes: '', shared_email: '' });
  }
  ...
  async function saveEditTask() {
    ...
    if (err) { setError(err.message); loadAll(); }
      ...
      const shareEmail = taskEditDraft.shared_email?.trim();
      if (shareEmail) {
        if (!isPremium) {
          setView('premium');
          setSaving('Premium richiesto');
        } else {
          const { error: shareErr } = await supabase.rpc('share_task_by_email', { task_id_input: id, recipient_email: shareEmail });
          if (shareErr) setError(shareErr.message);
          else setSaving('Modifica salvata e condivisa');
        }
  ...
  return <main className="app-shell">
    ...
    {view === 'planner' ? <>
      ...
      <PremiumPlannerTools isPremium={isPremium} categories={categories} newRecurring={newRecurring} setNewRecurring={setNewRecurring} addRecurringTask={addRecurringTask} recurringTasks={recurringTasks} deleteRecurringTask={deleteRecurringTask} generateRecurringTasks={generateRecurringTasks} />
    ...
    {isTaskEditSheetOpen && editingTask && <div className="task-edit-sheet-backdrop" onClick={cancelEditTask}>
      <div className="task-edit-sheet" onClick={e => e.stopPropagation()}>
        ...
        <TaskEditForm editDraft={taskEditDraft} setEditDraft={setTaskEditDraft} categories={categories} onSave={saveEditTask} onCancel={cancelEditTask} compact={false} shareAllowed={isPremium} />
      </div>
    </div>}
...
function TaskCard({ task, category, dragged, onPointerDown, onDelete, onEdit, editingTaskId, editDraft, setEditDraft, categories, onSaveEdit, onCancelEdit, isTaskEditSheetOpen }) {
  ...
  if (editing) return <TaskEditForm editDraft={editDraft} setEditDraft={setEditDraft} categories={categories} onSave={onSaveEdit} onCancel={onCancelEdit} compact={false} shareAllowed={true} />;
...
function PlannedTask({ task, category, dragged, onPointerDown, onArchive, onEdit, editingTaskId, editDraft, setEditDraft, categories, onSaveEdit, onCancelEdit, isTaskEditSheetOpen }) {
  ...
  const editing = editingTaskId === task.id && !isTaskEditSheetOpen;
  if (editing) return <div className="planned-task is-editing"><TaskEditForm editDraft={editDraft} setEditDraft={setEditDraft} categories={categories} onSave={onSaveEdit} onCancel={onCancelEdit} compact shareAllowed={false} /></div>;
...
function TaskEditForm({ editDraft, setEditDraft, categories, onSave, onCancel, compact, shareAllowed }) {
  return <div className={`task-edit-form ${compact ? 'compact' : ''}`} onPointerDown={e => e.stopPropagation()} onClick={e => e.stopPropagation()}>
    ...
    {!compact && <input value={editDraft.shared_email || ''} onChange={e => setEditDraft(d => ({ ...d, shared_email: e.target.value }))} placeholder={shareAllowed ? 'Condividi con email utente Premium' : 'Condivisione Premium'} />}
  ...
  return <section className="premium-tools-panel">
    ...
    {isPremium ? <div className="premium-tools-grid recurring-only-grid">
      <form className="recurring-form" onSubmit={addRecurringTask}>
        ...
        <input value={newRecurring.title} onChange={e => setNewRecurring(v => ({ ...v, title: e.target.value }))} placeholder="Es. Presenze da inserire" />
        ...
        <select value={newRecurring.recurrence_day} onChange={e => setNewRecurring(v => ({ ...v, recurrence_day: e.target.value }))}>{days.map(d => <option key={d} value={d}>Ogni {dayFullLabels[d]}</option>)}</select>
...
function PremiumStatsBlock({ stats, isPremium, reportSettings, toggleWeeklyReport }) {
  ...
  return <div className="panel premium-panel review-wide premium-stats-block">
    ...
    {!isPremium ? <div className="premium-locked-note compact-lock">Sblocca Premium per storico completo, archivio illimitato, report email, trend e statistiche evolute.</div> : <>
      <div className="advanced-stats-grid premium-expanded-stats">
        ...
        <div><strong>{stats.futureCount}</strong><span>Nel futuro</span></div>
        ...
        <div><strong>{reportSettings?.weekly_email_enabled ? 'Attivo' : 'Non attivo'}</strong><span>Report email</span></div>
      </div>
      <div className="premium-insight-layout">
        ...
        <div className="premium-insight-card"><div className="premium-insight-card-head"><strong>Procrastinazione tracciata</strong><span>Spostamenti attività</span></div><ProcrastinationMini stats={stats} /></div>
      </div>
      <div className="premium-insight-box">
        <strong>Nota sugli insight</strong>
        <span>Il trend e la heatmap usano le attività completate. La procrastinazione ora usa gli spostamenti reali tra giorni; se non ci sono ancora spostamenti, Prioro mostra una stima sulle attività aperte da oltre 7 giorni.</span>
      </div>
...
function PremiumPage({ isPremium, planLabel, activatePlan }) {
  return <section className="premium-page">
    <div className="stats-hero"><div><p className="eyebrow">Premium</p><h2>Free o Premium</h2><p>Il piano Free resta completo per pianificare. Premium aggiunge ricorrenze, storico evoluto, email report, statistiche avanzate e attività condivise.</p></div><span className={isPremium ? 'premium-badge active' : 'premium-badge'}>{planLabel}</span></div>
...
function HeatmapMini({ rows }) {
  const max = Math.max(1, ...rows.map(r => r.value));
  return <div className="heatmap-mini">{rows.map(r => { const level = r.value === 0 ? 0 : Math.ceil((r.value / max) * 4); return <span key={r.date} className={`heat-${level}`} title={`${r.label}: ${r.value} completate`} />; })}</div>;
}
