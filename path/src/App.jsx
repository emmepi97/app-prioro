...
function DualBarList({ rows, empty = 'Nessun dato disponibile.' }) {
  ...
  const barWidth = value => `${Math.max(value ? 4 : 0, (value / max) * 100)}%`;
  return <div className="dual-bar-list clean-dual-bar-list">{rows.length ? rows.map(r => <div className="dual-bar-row clean-dual-bar-row" key={r.label}>
    <div className="dual-bar-label clean-dual-bar-label"><strong>{r.label}</strong></div>
    <div className="dual-bars clean-dual-bars">
      <div className="dual-track-row"><div className="dual-track"><span className="bar-fill-value" style={{ width: barWidth(r.completed || 0) }}><i className="productive-bar" />{(r.completed || 0) > 0 && <b>{r.completed || 0}</b>}</span></div></div>
      <div className="dual-track-row"><div className="dual-track"><span className="bar-fill-value" style={{ width: barWidth(r.procrastinated || 0) }}><i className="procrastination-bar" />{(r.procrastinated || 0) > 0 && <b>{r.procrastinated || 0}</b>}</span></div></div>
    </div>
  </div>) : <div className="archive-empty">{empty}</div>}</div>;
}
function StatsFaq() {
...
function HeatmapMini({ rows, variant = 'green', label = 'completate' }) {
  const max = Math.max(1, ...rows.map(r => r.value));
  return <div className={`heatmap-mini ${variant === 'red' ? 'heatmap-red' : ''}`}>{rows.map(r => { const level = r.value === 0 ? 0 : Math.ceil((r.value / max) * 4); return <span key={r.date} className={`heat-${level}`} title={`${r.label}: ${r.value} ${label}`} />; })}</div>;
}
