...
function TaskCard({ task, category, dragged, onPointerDown, onDelete, onArchive, onEdit, editingTaskId, editDraft, setEditDraft, categories, onSaveEdit, onCancelEdit, isTaskEditSheetOpen }) {
  ...
  return <div className={`backlog-task ${dragged ? 'is-dragging' : ''}`} onPointerDown={e => onPointerDown(e, task.id, task)} onDoubleClick={() => onEdit(task)} title="Trascina per pianificare. Doppio click per modificare.">
    <button className="check backlog-check" onPointerDown={e => e.stopPropagation()} onClick={e => { e.stopPropagation(); onArchive(task.id); }} aria-label="Segna come fatta" />
