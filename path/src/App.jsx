  ...
  return <section className="premium-tools-panel">
    ...
    {isPremium ? <div className="premium-tools-grid recurring-only-grid">
      ...
      <div className="recurring-list-box">
        <div className="recurring-list-head"><div><strong>Ricorrenze settimanali</strong><p>Modelli salvati. Puoi generare la settimana o eliminarli.</p></div><button type="button" onClick={() => generateRecurringTasks(recurringTasks)}>Genera attività della settimana</button></div>
        {recurringTasks.length ? <div className="recurring-list">{recurringTasks.map(item => <div className="recurring-row" key={item.id}><div><b>{item.title}</b><span>Ogni {dayFullLabels[item.recurrence_day] || item.recurrence_day}</span></div><button type="button" onClick={() => deleteRecurringTask(item.id)}>Elimina ricorrenza</button></div>)}</div> : <div className="archive-empty">Nessuna ricorrenza creata.</div>}
      </div>
