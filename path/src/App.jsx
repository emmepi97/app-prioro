...
function Planner({ session }) {
  ...
  function pointerDown(e, id, task = null) {
    ...
    if (isMobile && task && previousTap.id === id && now - previousTap.time < 360) {
      e.preventDefault();
  ...
  return <main className="app-shell">
    ...
    {isTaskEditSheetOpen && editingTask && <div className="task-edit-sheet-backdrop" onClick={cancelEditTask}>
      <div className="task-edit-sheet" onClick={e => e.stopPropagation()}>
        ...
        <TaskEditForm editDraft={taskEditDraft} setEditDraft={setTaskEditDraft} categories={categories} goals={activeGoals} onSave={saveEditTask} onCancel={cancelEditTask} compact={false} />
      </div>
    </div>}
  </main>;
}

function DayColumn({ day, active, items, categoryMap, goalMap, hoverDay, draggedTaskId, pointerDown, archiveTask, clearDay, onEdit, editingTaskId, editDraft, setEditDraft, categories, goals, onSaveEdit, onCancelEdit, isTaskEditSheetOpen }) {
  return <div data-day={day} className={`day-column ${active ? 'mobile-active' : ''} ${hoverDay === day ? 'drag-over' : ''}`}><div className="day-title-row"><div className="day-title">{day}</div><button className="clear-day-btn" onClick={() => clearDay(day)} disabled={!items.length}>Svuota</button></div><div className="planned-list">{items.length ? items.map(t => <PlannedTask key={t.id} task={t} category={categoryMap[t.category_id]} goal={goalMap[t.goal_id]} dragged={draggedTaskId === t.id} onPointerDown={pointerDown} onArchive={archiveTask} onEdit={onEdit} editingTaskId={editingTaskId} editDraft={editDraft} setEditDraft={setEditDraft} categories={categories} goals={goals} onSaveEdit={onSaveEdit} onCancelEdit={onCancelEdit} isTaskEditSheetOpen={isTaskEditSheetOpen} />) : <Empty compact text="Trascina qui." />}</div></div>;
}

function TaskCard({ task, category, goal, dragged, onPointerDown, onDelete, onEdit, editingTaskId, editDraft, setEditDraft, categories, goals, onSaveEdit, onCancelEdit, isTaskEditSheetOpen }) {
...
function PlannedTask({ task, category, goal, dragged, onPointerDown, onArchive, onEdit, editingTaskId, editDraft, setEditDraft, categories, goals, onSaveEdit, onCancelEdit, isTaskEditSheetOpen }) {
