...
function Planner({ session }) {
  ...
  return <main className="app-shell">
    ...
    <section className="below-grid">
      ...
      <div className="panel categories-list-panel"><div className="section-title"><h2>Categorie</h2><span className="count">{categories.length}</span></div><div className="chips"><button className={!categoryFilter ? 'chip chip-active' : 'chip'} onClick={() => setCategoryFilter('')}>Tutte</button>{categories.map(c => <span key={c.id} className={categoryFilter === c.id ? 'chip chip-active category-chip' : 'chip category-chip'} style={{ '--chip': c.color }} onClick={() => setCategoryFilter(c.id)} title="Click per filtrare. Usa × per eliminare."><span className="category-chip-name">{c.name}</span><button type="button" className="category-delete-btn" aria-label={`Elimina categoria ${c.name}`} onClick={e => { e.stopPropagation(); deleteCategory(c.id); }}>×</button></span>)}</div></div>
