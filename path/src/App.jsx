...
function Planner({ session }) {
  ...
  useEffect(() => {
    function preventBrowserDrop(e) {
      e.preventDefault();
    }

    const options = { capture: true };
    document.addEventListener('dragover', preventBrowserDrop, options);
    document.addEventListener('drop', preventBrowserDrop, options);
    window.addEventListener('dragover', preventBrowserDrop, options);
    window.addEventListener('drop', preventBrowserDrop, options);

    return () => {
      document.removeEventListener('dragover', preventBrowserDrop, options);
      document.removeEventListener('drop', preventBrowserDrop, options);
      window.removeEventListener('dragover', preventBrowserDrop, options);
      window.removeEventListener('drop', preventBrowserDrop, options);
    };
  }, []);
  ...
  return (
    <div className="app-shell">
      ...
      <main className="app-main" onDragOver={e => e.preventDefault()} onDrop={e => e.preventDefault()}>
            ...
            <DropZone day="" dragId={dragId} onMove={moveTask} className="dropzone backlog-zone">
              <div className="backlog-list backlog-list-horizontal">
                {backlogTasks.length ? backlogTasks.map(task => (
                  <BacklogTask key={task.id} task={task} category={categoryMap[task.category_id]} goal={goalMap[task.goal_id]} setDragId={setDragId} onDelete={deleteTask} onEdit={editTaskTitle} />
          ...
          <section className="workflow-week">
            ...
            <div className="week-grid">
              {days.map((day, index) => {
                ...
                return (
                  <DropZone key={day} day={day} dragId={dragId} onMove={moveTask} className={`day-column ${index === selectedDayIndex ? 'mobile-active' : ''}`}>
                    ...
                    <div className="planned-list">
                      {dayTasks.length ? dayTasks.map(task => (
                        <PlannedTask key={task.id} task={task} category={categoryMap[task.category_id]} goal={goalMap[task.goal_id]} setDragId={setDragId} onArchive={archiveTask} onEdit={editTaskTitle} />
  ...
  return (
    <div
      ...
      onDrop={e => {
        e.preventDefault();
        setOver(false);
        const droppedId = e.dataTransfer?.getData('application/prioro-task-id') || e.dataTransfer?.getData('text/plain') || dragId;
...
function BacklogTask({ task, category, goal, setDragId, onDelete, onEdit }) {
  const cat = category || { name: 'Senza categoria', color: '#94a3b8' };

  return (
    <article className="backlog-task" draggable onDragStart={e => { e.stopPropagation(); e.dataTransfer.effectAllowed = 'move'; e.dataTransfer.setData('application/prioro-task-id', task.id); e.dataTransfer.setData('text/plain', task.id); setDragId(task.id); }} onDragEnd={() => setDragId(null)} onDoubleClick={() => onEdit(task)} title="Doppio click per modificare">
      <div>
...
function PlannedTask({ task, category, goal, setDragId, onArchive, onEdit }) {
  const cat = category || { name: 'Senza categoria', color: '#a1a1aa' };

  return (
    <div className="planned-task" draggable onDragStart={e => { e.stopPropagation(); e.dataTransfer.effectAllowed = 'move'; e.dataTransfer.setData('application/prioro-task-id', task.id); e.dataTransfer.setData('text/plain', task.id); setDragId(task.id); }} onDragEnd={() => setDragId(null)} onDoubleClick={() => onEdit(task)} title="Doppio click per modificare">
      <button type="button" draggable="false" className="check" onMouseDown={e => e.stopPropagation()} onClick={() => onArchive(task.id)} aria-label="Segna come fatta" />
  ...
  return (
    <article className="goal-card">
      <div className="goal-card-head">
        ...
        <button type="button" onClick={() => onArchive(goal.id)}>Archivia</button>
      </div>
