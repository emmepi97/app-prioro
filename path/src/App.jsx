...
function Prioro({ session }) {
  ...
  const stats = useMemo(() => buildStats(tasks, categories), [tasks, categories]);
  ...
  return <main className="app-shell">
    ...
    </> : <StatsPage stats={stats} categories={categories} categoryMap={categoryMap} moveTaskToFuture={moveTaskToFuture} />}
...
function StatsPage({ stats, categoryMap, moveTaskToFuture }) {
  return <section className="stats-page">
    <div className="stats-hero"><div><p className="eyebrow">Statistiche</p><h2>Review produttività</h2><p>Una lettura pulita di cosa hai fatto, cosa resta aperto e cosa conviene spostare nel futuro.</p></div></div>
...
function buildStats(tasks, categories) {
  const categoryMap = Object.fromEntries(categories.map(c => [c.id, c]));
  const completedTasks = tasks.filter(t => t.status === 'Fatto');
  ...
  completedTasks.forEach(t => { if (completedByDay[t.day] !== undefined) completedByDay[t.day] += 1; });
