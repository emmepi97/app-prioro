          ...
          <section className="workflow-week">
            <div className="calendar-head">
              <div>
                ...
                <p>Trascina le attività nei giorni. Doppio click su un'attività per modificarne il testo.</p>
              </div>
            </div>
            <div className="mobile-day-tabs">
              {days.map((day, index) => <button key={day} className={index === selectedDayIndex ? 'active' : ''} onClick={() => setSelectedDayIndex(index)}>{day.slice(0, 3)}</button>)}
            </div>
            <div className="week-grid" onTouchStart={handleWeekTouchStart} onTouchEnd={handleWeekTouchEnd}>
              {days.map((day, index) => {
...
function BacklogTask({ task, category, goal, setDragId, onDelete, onEdit }) {
  ...
  return (
    <article className="backlog-task" draggable onDragStart={() => setDragId(task.id)} onDoubleClick={() => onEdit(task)} title="Doppio click per modificare">
      <div>
...
function PlannedTask({ task, category, goal, setDragId, onArchive, onEdit }) {
  ...
  return (
    <div className="planned-task" draggable onDragStart={() => setDragId(task.id)} onDoubleClick={() => onEdit(task)} title="Doppio click per modificare">
