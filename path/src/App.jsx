...
function BacklogTask({ task, category, goal, setDragId, onDelete, onEdit }) {
  ...
  return (
    <article className="backlog-task" draggable onDragStart={e => { e.dataTransfer.effectAllowed = 'move'; e.dataTransfer.setData('text/plain', task.id); setDragId(task.id); }} onDragEnd={() => setDragId(null)} onDoubleClick={() => onEdit(task)} title="Doppio click per modificare">
      <div>
      ...
      <button type="button" onClick={() => onDelete(task.id)} title="Elimina"><Trash2 size={15} /></button>
    </article>
  );
}
...
function PlannedTask({ task, category, goal, setDragId, onArchive, onEdit }) {
  ...
  return (
    <div className="planned-task" draggable onDragStart={e => { e.dataTransfer.effectAllowed = 'move'; e.dataTransfer.setData('text/plain', task.id); setDragId(task.id); }} onDragEnd={() => setDragId(null)} onDoubleClick={() => onEdit(task)} title="Doppio click per modificare">
      <button type="button" className="check" onClick={() => onArchive(task.id)} aria-label="Segna come fatta" />
