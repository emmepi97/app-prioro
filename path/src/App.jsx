...
function DualBarList({ rows, empty = 'Nessun dato disponibile.' }) {
  ...
  return <div className="dual-bar-list">{rows.length ? rows.map(r => <div className="dual-bar-row" key={r.label}>
    <div className="dual-bar-label"><strong>{r.label}</strong><span><b>{r.completed || 0}</b> completate · <b>{r.procrastinated || 0}</b> procrastinate</span></div>
    <div className="dual-bars"><div className="dual-track"><i className="productive-bar" style={{ width: `${Math.max((r.completed || 0) ? 4 : 0, ((r.completed || 0) / max) * 100)}%` }} /></div><div className="dual-track"><i className="procrastination-bar" style={{ width: `${Math.max((r.procrastinated || 0) ? 4 : 0, ((r.procrastinated || 0) / max) * 100)}%` }} /></div></div>
