...
function taskTimeValue(task) {
  return normalizeTaskTime(task?.scheduled_time);
}
function sortScheduledTasks(list) {
...
const onboardingSteps = [
  {
    view: 'planner',
    title: 'Benvenuto in PrioWeek',
    text: 'PrioWeek ti aiuta a trasformare attività sparse in una settimana chiara. Parti creando le attività, poi le trascini nei giorni giusti.'
  },
  {
    view: 'planner',
    title: 'Crea e organizza le attività',
    text: 'Inserisci titolo, categoria, priorità e note. Le nuove attività finiscono nell’elenco attività, pronte da pianificare.'
  },
  {
    view: 'planner',
    title: 'Trascina nella settimana',
    text: 'Sposta le card nei giorni della settimana, in Ogni giorno o in Futuro. Se un’attività è già fatta, usa il check nella card.'
  },
  {
    view: 'stats',
    title: 'Controlla le statistiche',
    text: 'La sezione Statistiche ti mostra completate, aperte, giorni più produttivi, categorie e segnali di procrastinazione.'
  },
  {
    view: 'premium',
    title: 'Funzioni Premium',
    text: 'Premium sblocca insight evoluti, PrioWeek Score, ricorrenze, report e strumenti avanzati. Puoi usarlo quando il planner base inizia a starti stretto.'
  }
];
...
function MissingConfig() {
  return <div className="auth-page"><div className="auth-card"><p className="eyebrow">PrioWeek</p><h1>Config Supabase mancante</h1><p>{supabaseConfigError}</p></div></div>;
}
...
function AuthGate() {
  ...
  const demoInitialTasks = ['Preparare presentazione', 'Chiamare cliente', 'Fare la spesa', 'Allenamento palestra'];
  const demoInitialWeek = { LUN: ['Riunione team'], MAR: ['Preparare report'], MER: ['Chiamare cliente'], GIO: ['Fare la spesa'], VEN: ['Allenamento palestra'] };
  const [demoTasks, setDemoTasks] = useState(demoInitialTasks);
  const [demoWeek, setDemoWeek] = useState(demoInitialWeek);
  ...
  function dropDemoTask(day, title) {
    if (!title) return;
    setDemoTasks(items => items.filter(item => item !== title));
    setDemoWeek(week => {
      const cleaned = Object.fromEntries(Object.entries(week).map(([key, items]) => [key, items.filter(item => item !== title)]));
      return { ...cleaned, [day]: cleaned[day].includes(title) ? cleaned[day] : [...cleaned[day], title] };
    });
  }

  function returnDemoTask(title) {
    if (!title) return;
    setDemoWeek(week => Object.fromEntries(Object.entries(week).map(([key, items]) => [key, items.filter(item => item !== title)])));
    setDemoTasks(items => items.includes(title) ? items : [...items, title]);
  }

  function resetDemo() {
    setDemoTasks(demoInitialTasks);
    setDemoWeek(demoInitialWeek);
  }
    ...
    return <div className="landing-page landing-pro">
      <header className="landing-nav landing-pro-nav">
        <div className="landing-brand-wrap">
          <div className="landing-logo landing-pro-logo">PrioWeek</div>
          <span>Planner settimanale smart</span>
        </div>
        <div className="landing-nav-actions">
          <button type="button" onClick={() => { setMode('login'); setAuthOpen(true); }}>Accedi</button>
          <button type="button" className="landing-nav-primary" onClick={() => { setMode('register'); setAuthOpen(true); }}>Inizia gratis</button>
        </div>
      </header>

      <main className="landing-pro-main">
        <section className="landing-v42-hero">
          <div className="landing-pill">Planner settimanale · demo interattiva</div>
          <h1>Organizza la settimana senza perdere il controllo.</h1>
          <p>Scrivi le attività, trascinale nella settimana e misura come stai lavorando. Guarda la demo, poi crea il tuo planner gratis.</p>
          <div className="landing-v42-actions">
            <button type="button" className="primary" onClick={() => { setMode('register'); setAuthOpen(true); }}>Inizia gratis</button>
            <button type="button" className="soft-btn" onClick={() => { setMode('login'); setAuthOpen(true); }}>Accedi</button>
          </div>
        </section>

        <section className="landing-v42-demo" aria-label="Demo PrioWeek">
          <div className="demo-main-card">
            <div className="demo-main-head">
              <div><p className="eyebrow">Demo</p><h2>Prova a organizzare una settimana.</h2><span>Trascina una card su un giorno oppure riportala nell'elenco attività.</span></div>
              <button type="button" onClick={resetDemo}>Reset demo</button>
            </div>
            <div className="demo-workspace">
              <div className="demo-backlog" onDragOver={event => event.preventDefault()} onDrop={event => returnDemoTask(event.dataTransfer.getData('text/plain'))}>
                <h3>Elenco attività</h3>
                <p>Attività ancora da pianificare.</p>
                {demoTasks.map(task => <div key={task} className="demo-task" draggable onDragStart={event => event.dataTransfer.setData('text/plain', task)}>{task}</div>)}
                {!demoTasks.length && <div className="demo-empty">Tutte le attività sono nella settimana.</div>}
              </div>
              <div className="demo-week-board">
                {Object.entries(demoWeek).map(([day, items]) => <div key={day} className="demo-day" onDragOver={event => event.preventDefault()} onDrop={event => dropDemoTask(day, event.dataTransfer.getData('text/plain'))}>
                  <strong>{day}</strong>
                  {items.map(item => <span key={item} draggable onDragStart={event => event.dataTransfer.setData('text/plain', item)}>{item}</span>)}
                  {!items.length && <em>Trascina qui</em>}
                </div>)}
              </div>
            </div>
          </div>
          <div className="demo-kpi-row">
            <div><b>91</b><span>PrioWeek Score</span></div>
            <div><b>87%</b><span>Attività completate</span></div>
            <div><b>12%</b><span>Procrastinazione</span></div>
          </div>
        </section>

        <section className="landing-v43-audience">
          <div><strong>Professionisti</strong><span>Gestisci lavoro, riunioni e scadenze.</span></div>
          <div><strong>Studenti</strong><span>Organizza studio, consegne ed esami.</span></div>
          <div><strong>Persone impegnate</strong><span>Tieni insieme attività personali, lavoro e abitudini.</span></div>
        </section>

        <section className="landing-v44-start">
          <div>
            <p className="eyebrow">Prima di iniziare</p>
            <h2>Dal caos della settimana a un piano chiaro.</h2>
            <p>PrioWeek ti aiuta a trasformare un elenco di attività in una settimana ordinata, leggibile e sostenibile.</p>
          </div>
          <div className="landing-v44-points">
            <span>Drag & Drop immediato</span>
            <span>Planner settimanale</span>
            <span>Statistiche produttività</span>
            <span>Versione gratuita disponibile</span>
          </div>
          <button type="button" className="primary" onClick={() => { setMode('register'); setAuthOpen(true); }}>Inizia gratis</button>
        </section>

        <section className="landing-v42-final">
          <div>
            <p className="eyebrow">Inizia ora</p>
            <h2>Prendi il controllo della tua settimana.</h2>
          </div>
          <button type="button" className="primary" onClick={() => { setMode('register'); setAuthOpen(true); }}>Crea account gratuito</button>
        </section>

        {authOpen && <div className="auth-modal-backdrop" onClick={() => setAuthOpen(false)}>
          <div className="auth-modal-card" onClick={event => event.stopPropagation()}>
            <button type="button" className="auth-modal-close" onClick={() => setAuthOpen(false)}>Chiudi</button>
            <p className="eyebrow">{mode === 'login' ? 'Accesso' : 'Registrazione'}</p>
            <h2>{mode === 'login' ? 'Bentornato' : 'Crea il tuo planner'}</h2>
            <p>{mode === 'login' ? 'Accedi e continua a organizzare la tua settimana.' : 'Crea un account gratuito e prova subito PrioWeek.'}</p>
  ...
  return <PrioWeek session={session} />;
}

function PrioWeek({ session }) {
  ...
  const [taskEditDraft, setTaskEditDraft] = useState({ title: '', category_id: '', priority: '', notes: '', scheduled_time: '' });
  ...
  useEffect(() => {
    if (loading) return;
    const key = `prioweek_onboarding_done_${user.id}`;
  ...
  async function addTask(e) {
    ...
    const { data, error: err } = await supabase.from('tasks').insert({
      ...
      notes: newNotes.trim(),
      scheduled_time: normalizeTaskTime(newTime) || null,
      day: '',
      status: 'Da fare'
    }).select('*').single();
  ...
  async function moveTask(id, day) {
    ...
    setSaving('Salvataggio...');
    const timePatch = days.includes(toDay) ? {} : { scheduled_time: null };
    setTasks(p => p.map(t => t.id === id ? { ...t, day: toDay, ...timePatch } : t));
    const { error: err } = await supabase.from('tasks').update({ day: toDay, ...timePatch }).eq('id', id);
  ...
  async function archiveTask(id) {
    ...
    const currentTask = tasks.find(t => t.id === id);
    const completedDay = days.includes(currentTask?.day) ? currentTask.day : getTodayDayKey();
    const patch = { status: 'Fatto', day: completedDay };
    setTasks(p => p.map(t => t.id === id ? { ...t, ...patch, updated_at: today } : t));
    const { error: err } = await supabase.from('tasks').update(patch).eq('id', id);
    if (err) { setError(err.message); loadAll(); }
    else setSaving(`Completata e conteggiata su ${dayFullLabels[completedDay] || completedDay}`);
  }
  ...
  async function restoreTask(id) {
    setTasks(p => p.map(t => t.id === id ? { ...t, status: 'Da fare', day: '', scheduled_time: null } : t));
    const { error: err } = await supabase.from('tasks').update({ status: 'Da fare', day: '', scheduled_time: null }).eq('id', id);
    if (err) { setError(err.message); loadAll(); }
  }
  ...
  async function clearDay(day) {
    ...
    setTasks(p => p.map(t => ids.includes(t.id) ? { ...t, day: '', scheduled_time: null } : t));
    const { error: err } = await supabase.from('tasks').update({ day: '', scheduled_time: null }).in('id', ids);
    if (err) { setError(err.message); loadAll(); }
  }

  async function autoRolloverOverdueTasks() {
    const todayKey = getLocalDateKey();
    const storageKey = `prioweek_rollover_${user.id}_${todayKey}`;
  ...
  function startEditTask(task) {
    ...
    setTaskEditDraft({
      ...
      notes: task.notes || '',
      scheduled_time: taskTimeValue(task)
    });
  ...
  function cancelEditTask() {
    ...
    setTaskEditDraft({ title: '', category_id: '', priority: '', notes: '', scheduled_time: '' });
  }
  ...
  async function saveEditTask() {
    ...
    const patch = {
      ...
      scheduled_time: normalizeTaskTime(taskEditDraft.scheduled_time) || null
    };
    setEditingTaskId('');
    setIsTaskEditSheetOpen(false);
  ...
  function exportExcel() {
    const rows = tasks.map(t => `<tr><td>${esc(t.title)}</td><td>${esc(categoryMap[t.category_id]?.name || '')}</td><td>${esc(t.priority)}</td><td>${esc(t.day || 'NON PIANIFICATO')}</td><td>${esc(taskTimeValue(t) || '')}</td><td>${esc(t.status)}</td><td>${esc(t.notes || '')}</td></tr>`).join('');
    downloadFile('prioweek-attivita.xls', 'application/vnd.ms-excel', `<table><tr><th>Titolo</th><th>Categoria</th><th>Priorità</th><th>Giorno</th><th>Orario</th><th>Stato</th><th>Note</th></tr>${rows}</table>`);
  }
  ...
  function downloadPlannerPdf() {
    ...
    const byDay = day => sortScheduledTasks(active.filter(t => t.day === day));
    const every = sortScheduledTasks(active.filter(t => t.day === 'OGNI_GIORNO'));
    const future = sortScheduledTasks(active.filter(t => t.day === 'FUTURO'));
    ...
    function drawMiniTask(task, x, y, w, mode = 'card') {
      ...
      if (mode === 'line') {
        ...
        text(`${taskTimeValue(task) ? taskTimeValue(task) + ' · ' : ''}${truncate(task.title, 25)}`, x + 4, y, { size: 5.4, color: '#111827' });
      ...
      text(`${taskTimeValue(task) ? taskTimeValue(task) + ' · ' : ''}${truncate(task.title, 28)}`, x + 2, y + 5, { size: 6.1, bold: true });
      doc.setFillColor(r, g, b);
    ...
    function drawCompactTask(task, x, y, w, h) {
      ...
      text(`${taskTimeValue(task) ? taskTimeValue(task) + ' · ' : ''}${truncate(task.title, 24)}`, x + 2, y + 4.6, { size: 5.6, bold: true });
      doc.setFillColor(r, g, b);
    ...
    text('PrioWeek - Planner settimanale', margin, 12, { size: 14, bold: true });
    ...
    doc.save('prioweek-planner-settimanale.pdf');
  }

  function finishOnboarding() {
    window.localStorage.setItem(`prioweek_onboarding_done_${user.id}`, '1');
  ...
  function previousOnboardingStep() {
    const previousStep = Math.max(0, onboardingStep - 1);
    setOnboardingStep(previousStep);
    setView(onboardingSteps[previousStep].view);
  }
  ...
  return <main className="app-shell">
    <div className="app-sticky-head">
      <header className="topbar unified-menu">
        <div className="brand-block"><h1>PrioWeek</h1><p className="subtitle">Organizza la tua settimana · {user.email}</p></div>
    ...
    {view === 'planner' ? <>
      <section className="planner-hero">
        ...
        <form className="quick-form-wide no-goals planner-hero-form" onSubmit={addTask}>
          ...
          <input className="time-input" type="time" value={newTime} onChange={e => setNewTime(e.target.value)} title="Orario opzionale" />
...
function TaskCard({ task, category, dragged, onPointerDown, onDelete, onArchive, onEdit, editingTaskId, editDraft, setEditDraft, categories, onSaveEdit, onCancelEdit, isTaskEditSheetOpen }) {
  ...
  return <div className={`backlog-task has-inline-check ${dragged ? 'is-dragging' : ''}`} onPointerDown={e => onPointerDown(e, task.id, task)} onDoubleClick={() => onEdit(task)} title="Trascina per pianificare. Doppio click per modificare.">
    ...
    <div className="backlog-task-main"><strong>{task.title}</strong><div className="backlog-meta"><span style={{ '--dot': cat.color }}>{cat.name}</span><span>{task.priority}</span>{taskTimeValue(task) && <span className="task-time-chip">{taskTimeValue(task)}</span>}</div>{task.notes && <p>{task.notes}</p>}</div>
...
function PlannedTask({ task, category, dragged, onPointerDown, onArchive, onEdit, editingTaskId, editDraft, setEditDraft, categories, onSaveEdit, onCancelEdit, isTaskEditSheetOpen }) {
  ...
  return <div className={`planned-task ${dragged ? 'is-dragging' : ''}`} onPointerDown={e => onPointerDown(e, task.id, task)} onDoubleClick={() => onEdit(task)} title="Trascina per spostare. Doppio click per modificare."><button className="check" onClick={e => { e.stopPropagation(); onArchive(task.id); }} aria-label="Segna come fatta" /><div className="planned-content"><span className="planned-title-row">{taskTimeValue(task) && <span className="planned-time">{taskTimeValue(task)}</span>}<span className="planned-title">{task.title}</span></span><span className="planned-category" style={{ '--cat-color': cat.color }}>{cat.name}</span></div></div>;
}
...
function TaskEditForm({ editDraft, setEditDraft, categories, onSave, onCancel, compact }) {
  return <div className={`task-edit-form ${compact ? 'compact' : ''}`} onPointerDown={e => e.stopPropagation()} onClick={e => e.stopPropagation()}>
    ...
    <input className="time-input" type="time" value={editDraft.scheduled_time || ''} onChange={e => setEditDraft(d => ({ ...d, scheduled_time: e.target.value }))} title="Orario opzionale" />
...
function PremiumStatsBlock({ stats, isPremium, reportSettings, toggleWeeklyReport }) {
  ...
  return <div className="panel premium-panel review-wide premium-stats-block">
    ...
    {!isPremium ? <div className="premium-locked-note compact-lock">Sblocca Premium per PrioWeek Score, streak, trend, procrastinazione e statistiche evolute.</div> : <>
      <div className="prioweek-score-panel">
        <div className="prioweek-score-main"><span>PrioWeek Score</span><strong>{stats.prioWeekScore}</strong><small>/100</small></div>
        <div className="prioweek-score-grid">
...
function OnboardingWizard({ step, steps, onNext, onBack, onSkip }) {
  const current = steps[step];
  const isLast = step === steps.length - 1;
  return <div className="onboarding-backdrop" role="dialog" aria-modal="true" aria-label="Tutorial PrioWeek">
    <div className="onboarding-card">
      <div className="onboarding-head">
        <div><p className="eyebrow">Tutorial</p><h2>{current.title}</h2></div>
        <button type="button" onClick={onSkip} aria-label="Chiudi tutorial">×</button>
      </div>
      <p className="onboarding-text">{current.text}</p>
      <div className="onboarding-progress" aria-label={`Step ${step + 1} di ${steps.length}`}>
        {steps.map((_, index) => <span key={index} className={index <= step ? 'active' : ''} />)}
      </div>
      <div className="onboarding-actions">
        <button type="button" className="link-btn" onClick={onSkip}>Salta</button>
        <div>
          <button type="button" className="soft-btn" onClick={onBack} disabled={step === 0}>Indietro</button>
          <button type="button" className="primary" onClick={onNext}>{isLast ? 'Inizia' : 'Avanti'}</button>
        </div>
      </div>
    </div>
  </div>;
...
function StatsFaq() {
  return <div className="panel premium-panel stats-faq-panel"><h3>FAQ statistiche</h3><p className="stat-muted">Legenda rapida per leggere correttamente KPI e grafici.</p><div className="stats-faq-list">
    <details open><summary>Cos'è il PrioWeek Score?</summary><p>È una metrica interna da 0 a 100 che sintetizza completamento, continuità, trend settimanale e procrastinazione. Serve come indicatore rapido, non come giudizio assoluto.</p></details>
    ...
    <details><summary>Come si legge il tasso di procrastinazione?</summary><p>Indica il peso dei rimandi reali rispetto alle attività del periodo. Non conta le attività non pianificate che vengono semplicemente inserite o completate nel giorno corrente.</p></details>
    ...
    <details><summary>Cosa indicano le barre nere e rosse?</summary><p>La barra nera indica produttività, cioè attività completate. La barra rosso chiaro indica procrastinazione, cioè attività già pianificate e poi spostate o rimandate.</p></details>
    ...
    <details><summary>Cosa mostra la heatmap procrastinazione?</summary><p>Mostra in quali giorni hai rimandato attività già pianificate negli ultimi 35 giorni.</p></details>
...
function buildStats(tasks, categories, period = '30', movements = []) {
  ...
  const procrastinationHeatmap = Array.from({ length: 35 }, (_v, i) => {
    const d = new Date(now); d.setDate(d.getDate() - (34 - i)); d.setHours(0,0,0,0);
    const next = new Date(d); next.setDate(next.getDate() + 1);
    const value = procrastinationMovements.filter(m => { const ref = new Date(m.moved_at || m.created_at || 0); return ref >= d && ref < next; }).length;
    return { date: d.toISOString().slice(0,10), label: d.toLocaleDateString('it-IT', { day: '2-digit', month: '2-digit' }), value };
  });
  ...
  const isRealProcrastinationMovement = m => days.includes(m.from_day);
  const procrastinationMovements = movements.filter(isRealProcrastinationMovement);
  const periodMovements = procrastinationMovements.filter(m => {
    if (!fromDate) return true;
  ...
  periodMovements.forEach(m => {
    ...
    const movementDay = days.includes(m.from_day) ? m.from_day : null;
  ...
  const prioWeekScore = Math.round((completionRate * 0.4) + (procrastinationScore * 0.25) + (streakScore * 0.2) + (trendScore * 0.15));
  return {
