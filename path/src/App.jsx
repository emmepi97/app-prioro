      ...
      <main className="landing-pro-main">
        <section className="landing-pro-hero">
          <div className="landing-pro-copy">
            ...
            <p className="landing-lead">PrioWeek trasforma attività sparse, ricorrenze e cose rimandate in un planner settimanale chiaro, veloce e misurabile.</p>
        ...
        <section className="landing-pro-preview" aria-label="Anteprima prodotto">
          ...
          <div className="preview-card-small"><strong>PrioWeek Score</strong><b>82</b><span>/100</span></div>
...
function PrioWeek({ session }) {
  ...
  const todayTasks = filteredTasks.filter(t => t.day === todayDay).slice(0, 4);
  const scheduledWeekTasks = activeTasks.filter(t => days.includes(t.day));
  const weekCompletionRate = (doneTasks.length + scheduledWeekTasks.length) ? Math.round((doneTasks.length / (doneTasks.length + scheduledWeekTasks.length)) * 100) : 0;
  const highPriorityOpen = activeTasks.filter(t => t.priority === 'Alta').length;
  const highPriorityUnplanned = activeTasks.filter(t => t.priority === 'Alta' && !t.day).length;
  const emptyDays = days.filter(d => !activeTasks.some(t => t.day === d));
  const dominantCategory = (() => {
    const counts = {};
    activeTasks.forEach(t => { const key = t.category_id || 'none'; counts[key] = (counts[key] || 0) + 1; });
    const top = Object.entries(counts).sort((a, b) => b[1] - a[1])[0];
    if (!top) return { label: '-', count: 0, percent: 0 };
    return {
      label: top[0] === 'none' ? 'Senza categoria' : categoryMap[top[0]]?.name || 'Categoria rimossa',
      count: top[1],
      percent: activeTasks.length ? Math.round((top[1] / activeTasks.length) * 100) : 0
    };
  })();
  const weekDayLoad = days.map(d => ({ key: d, label: dayLabels[d], count: activeTasks.filter(t => t.day === d).length }));
  const maxDayLoad = Math.max(1, ...weekDayLoad.map(d => d.count));
  ...
  return <main className="app-shell">
    ...
    {view === 'planner' ? <>
      <PlannerDashboard
        ...
        doneTasks={doneTasks}
        scheduledWeekTasks={scheduledWeekTasks}
        todayTasks={todayTasks}
        highPriorityOpen={highPriorityOpen}
        highPriorityUnplanned={highPriorityUnplanned}
        futureTasks={futureTasks}
        emptyDays={emptyDays}
        weekDayLoad={weekDayLoad}
        maxDayLoad={maxDayLoad}
        dominantCategory={dominantCategory}
        prioWeekScore={stats.prioWeekScore}
...
function PlannerDashboard({ user, activeTasks, doneTasks, scheduledWeekTasks, todayTasks, highPriorityOpen, highPriorityUnplanned, futureTasks, emptyDays, weekDayLoad, maxDayLoad, dominantCategory, prioWeekScore, weekCompletionRate, setView }) {
  const firstName = (user.email || '').split('@')[0]?.split(/[._-]/)[0] || 'utente';
  const openCount = activeTasks.length;
  const completedCount = doneTasks.length;
  const alerts = [
    futureTasks.length ? `${futureTasks.length} attività in Futuro` : null,
    highPriorityUnplanned ? `${highPriorityUnplanned} priorità alte da pianificare` : null,
    emptyDays.length ? `${emptyDays.length} giorni senza attività pianificate` : null
  ].filter(Boolean);
  return <section className="home-dashboard">
    <div className="home-dashboard-hero">
      <div>
        <p className="eyebrow">Home</p>
        <h2>Buongiorno {firstName}</h2>
        <p>Hai <strong>{scheduledWeekTasks.length}</strong> attività pianificate questa settimana. Completa, sposta o ripianifica senza perdere il quadro generale.</p>
      </div>
      <div className="home-score-card">
        <span>PrioWeek Score</span>
        <strong>{prioWeekScore}</strong>
        <small>/100</small>
      </div>
    </div>
    <div className="home-progress-block">
      <div><b>Avanzamento settimana</b><span>{weekCompletionRate}% completato</span></div>
      <div className="home-progress-track"><i style={{ width: `${Math.min(100, Math.max(0, weekCompletionRate))}%` }} /></div>
    </div>
    <div className="home-kpi-grid">
      <div><strong>{openCount}</strong><span>Da fare</span></div>
      <div><strong>{completedCount}</strong><span>Completate</span></div>
      <div><strong>{highPriorityOpen}</strong><span>Priorità alta</span></div>
      <div><strong>{futureTasks.length}</strong><span>In Futuro</span></div>
    </div>
    <div className="home-info-grid">
      <div className="home-info-card">
        <h3>Oggi</h3>
        {todayTasks.length ? <ul>{todayTasks.map(t => <li key={t.id}>{t.title}</li>)}</ul> : <p>Nessuna attività programmata per oggi.</p>}
      </div>
      <div className="home-info-card">
        <h3>Attenzione</h3>
        {alerts.length ? <ul>{alerts.map(a => <li key={a}>{a}</li>)}</ul> : <p>Settimana pulita. Nessun alert rilevante.</p>}
      </div>
      <div className="home-info-card home-day-load">
        <h3>Carico per giorno</h3>
        {weekDayLoad.map(d => <div className="home-load-row" key={d.key}><span>{d.label}</span><i style={{ width: `${Math.max(d.count ? 8 : 0, (d.count / maxDayLoad) * 100)}%` }} /><b>{d.count || ''}</b></div>)}
      </div>
      <div className="home-info-card">
        <h3>Categoria principale</h3>
        <strong className="home-category-main">{dominantCategory.label}</strong>
        <p>{dominantCategory.count} attività · {dominantCategory.percent}% del carico attuale</p>
        <button type="button" className="soft-btn" onClick={() => setView('stats')}>Vedi statistiche</button>
      </div>
    </div>
  </section>;
}
