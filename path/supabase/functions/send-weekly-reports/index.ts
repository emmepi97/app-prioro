...
// Env required: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, RESEND_API_KEY, PRIOWEEK_FROM_EMAIL
// Schedule idea: every Monday at 07:00.
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2';

const supabase = createClient(Deno.env.get('SUPABASE_URL')!, Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!);
const resendKey = Deno.env.get('RESEND_API_KEY')!;
const fromEmail = Deno.env.get('PRIOWEEK_FROM_EMAIL') || 'PrioWeek <report@prioweek.com>';
...
Deno.serve(async () => {
  ...
  for (const row of settings || []) {
    ...
    const html = `<h2>Il tuo report PrioWeek</h2><p>Periodo: ${start.toLocaleDateString('it-IT')} - ${end.toLocaleDateString('it-IT')}</p><p><b>Completate:</b> ${completed.length}</p><p><b>Aperte:</b> ${open.length}</p><p><b>Completion rate:</b> ${completionRate}%</p><h3>Cose completate</h3><ul>${completed.slice(0, 20).map((task) => `<li>${task.title}</li>`).join('')}</ul>`;

    await fetch('https://api.resend.com/emails', {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${resendKey}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ from: fromEmail, to: email, subject: 'Il tuo report settimanale PrioWeek', html })
    });
    sent++;
  }
