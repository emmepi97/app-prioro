...
-- Premium cleanup v2: anti-duplicati ricorrenze + attività condivise
...
alter table public.tasks add column if not exists shared_from_user_id uuid references auth.users(id) on delete set null;
alter table public.tasks add column if not exists shared_original_task_id uuid references public.tasks(id) on delete cascade;
alter table public.tasks add column if not exists shared_with_email text;

create unique index if not exists ux_tasks_shared_copy
on public.tasks(user_id, shared_original_task_id)
where shared_original_task_id is not null;

create or replace function public.share_task_by_email(task_id_input uuid, recipient_email text)
returns uuid
language plpgsql
security definer
...
begin
  ...
  if source_task.id is null then
    raise exception 'Attività non trovata o non autorizzata';
  end if;
  ...
  if recipient_id is null then
    raise exception 'Utente destinatario non trovato';
  end if;

  if recipient_id = auth.uid() then
    raise exception 'Non puoi condividere un’attività con te stesso';
  end if;
  ...
  insert into public.tasks (
    ...
    source_task.day,
    'Da fare',
    coalesce(source_task.notes, '') || E'\nCondivisa da: ' || (select email from auth.users where id = auth.uid()),
    auth.uid(),
    source_task.id,
    lower(trim(recipient_email))
  )
  on conflict (user_id, shared_original_task_id)
  do update set
