...
declare con record;
begin
  for con in select conname from pg_constraint where conrelid = 'public.tasks'::regclass and contype = 'c' and pg_get_constraintdef(oid) ilike '%day%' loop
    execute format('alter table public.tasks drop constraint %I', con.conname);
  end loop;
end $$;
alter table public.tasks add constraint tasks_day_check check (day in ('','LUNEDI','MARTEDI','MERCOLEDI','GIOVEDI','VENERDI','SABATO','DOMENICA','OGNI_GIORNO'));

do $$
declare con record;
begin
  for con in select conname from pg_constraint where conrelid = 'public.goals'::regclass and contype = 'c' and pg_get_constraintdef(oid) ilike '%horizon%' loop
    execute format('alter table public.goals drop constraint %I', con.conname);
  end loop;
end $$;
alter table public.goals add constraint goals_horizon_check check (horizon in ('','Questo mese','Prossimi 3 mesi','Quest''anno'));
...
-- Update Prioro 24-07-2026: gli obiettivi devono sempre avere una tempistica.
update public.goals set horizon = 'Questo mese' where horizon = '' or horizon is null;
alter table public.goals alter column horizon set default 'Questo mese';
do $$
declare con record;
begin
  for con in
...
alter table public.goals add constraint goals_horizon_required_check
  check (horizon in ('Questo mese','Prossimi 3 mesi','Quest''anno'));

-- Update Prioro 24-07-2026: sezione FUTURO per attività da ricordare e fare prossimamente.
do $$
declare con record;
begin
  for con in
